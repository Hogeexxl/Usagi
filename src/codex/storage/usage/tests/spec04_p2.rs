...
fn set_active_source_proof(fixture: &Fixture, source_id: i64, active_offset: i64, guard_byte: u8) {
    ...
    connection
        .execute(
            "UPDATE codex_source_files SET observed_size=?2,observed_mtime_ns=1
...
fn carry_phase(fixture: &Fixture) -> UsageCarryPhase {
    with_codex(&fixture.ledger, |storage| {
        storage.load_usage_scan_state(&[1, 2], crate::codex::normalization::USAGE_PARSER_VERSION)
    })
    .unwrap()
    .plans
    .into_iter()
...
fn advance_carry_to(fixture: &Fixture, target: UsageCarryPhase) {
    for step in 0..32_i64 {
        ...
        with_codex(&fixture.ledger, |storage| {
            storage.resume_carry(1, 100 + step)
        })
        .unwrap();
    }
...
fn present_carry_source(
    ...
    active_offset: i64,
    guard_matches: bool,
) -> crate::codex::domain::SourceOutcome {
...
fn present_carry_source_with_size(
    ...
    with_codex(&fixture.ledger, |storage| {
        storage.record_source_observations_with_usage_carry_proofs(
            crate::codex::domain::SourceObservationBatch::new(
                observations,
...
fn resume_carry_until_present_finalized(fixture: &Fixture) {
    for step in 0..32_i64 {
        match with_codex(&fixture.ledger, |storage| {
            storage.resume_carry(1, 600 + step)
        })
        .unwrap()
        {
...
fn t_s04_048_planner_conflict_priority_matrix() {
    ...
    let parser_bump = with_codex(&fixture.ledger, |storage| {
        storage.load_usage_scan_state(&[1], crate::codex::normalization::USAGE_PARSER_VERSION + 1)
    })
    .unwrap();
    assert_eq!(
        parser_bump
    ...
    assert_eq!(
        with_codex(&relationship.ledger, |storage| storage
            .load_usage_scan_state(
                &[10],
                crate::codex::normalization::USAGE_PARSER_VERSION
            ))
        .unwrap()
    ...
    .unwrap();
    {
        ...
        connection
            .execute(
                "UPDATE codex_source_files SET observed_size=10 WHERE source_file_id=20",
                [],
            )
            .unwrap();
    }
    assert_eq!(
        with_codex(&offset_gt_raw.ledger, |storage| storage
            .load_usage_scan_state(
                &[20],
                crate::codex::normalization::USAGE_PARSER_VERSION
            ))
    ...
    .unwrap();
    {
        ...
        connection
            .execute(
                "UPDATE codex_source_files SET observed_size=20 WHERE source_file_id=30",
                [],
            )
            .unwrap();
        connection
            .execute(
                "UPDATE codex_usage_source_states SET observed_raw_size=20,raw_tail_status='unverified',
    ...
    assert_eq!(
        with_codex(&verify_tail.ledger, |storage| storage
            .load_usage_scan_state(
                &[30],
                crate::codex::normalization::USAGE_PARSER_VERSION
            ))
    ...
    .unwrap();
    assert_eq!(
        with_codex(&incremental.ledger, |storage| storage
            .load_usage_scan_state(
                &[40],
                crate::codex::normalization::USAGE_PARSER_VERSION
            ))
...
fn t_s04_048_parser_six_rebuilds_parser_five_epoch() {
    ...
    assert_eq!(
        crate::codex::normalization::canonical_algorithm_for(5),
        Some(4)
    );
    assert_eq!(
        crate::codex::normalization::canonical_algorithm_for(6),
        Some(5)
    );
}
...
fn t_s04_049_carry_four_phase_present_resume_is_exact_and_complete_only_finishes() {
    for target in [
        ...
        assert_eq!(
            ...
            crate::codex::domain::BuildDisposition::CarryResumedPresent,
            "phase {target:?}"
        );
        assert_eq!(
            carry_phase(&fixture),
        ...
        let plan = with_codex(&fixture.ledger, |storage| {
            storage
                .load_usage_scan_state(&[1, 2], crate::codex::normalization::USAGE_PARSER_VERSION)
        })
        .unwrap()
        .plans
        .into_iter()
        ...
        let carried: (i64, i64, String, i64) = connection
            .query_row(
                "SELECT
                    ...
                    (SELECT c.processing_status FROM codex_source_checkpoints c
                     WHERE c.source_file_id=1 AND c.consumer_kind='usage'),
                    (SELECT c.committed_offset FROM codex_source_checkpoints c
                     WHERE c.source_file_id=1 AND c.consumer_kind='usage')
        ...
        with_codex(&fixture.ledger, |storage| storage.complete_only(1, 800)).unwrap();
        let connection = fixture.ledger.connection().unwrap();
...
fn present_growth_then_missing_blocks_carry_finalize_with_unverified_tail() {
    ...
    let source_two = crate::codex::domain::SourceObservation::new(
        fixture_path("usage-2.jsonl"),
        crate::codex::domain::SourceArea::Sessions,
        22,
        22,
    ...
    for step in 0..32_i64 {
        match with_codex(&fixture.ledger, |storage| {
            storage.resume_carry(1, 700 + step)
        })
        .unwrap()
        {
            CarryStepOutcome::Progress => {}
    ...
    let proof: (String, Option<String>, String, i64, String, i64) = connection
        .query_row(
            "SELECT b.completion_status,b.completion_error_code,b.carry_phase,
             ...
             JOIN codex_source_checkpoints c ON c.source_file_id=b.source_file_id
                AND c.consumer_kind='usage'
             JOIN codex_source_files s ON s.source_file_id=b.source_file_id
    ...
    for (target, mismatch) in [
        ...
        let (device, inode, root) = match mismatch {
            Mismatch::Generation => {
                ...
                connection
                    .execute(
                        "UPDATE codex_source_files SET file_generation=file_generation+1
                         WHERE source_file_id=1",
                        [],
                    )
                    .unwrap();
        ...
        let proof = UsageCarryObservationProof {
            device_id: device,
            inode,
            active_committed_offset: active_offset,
        ...
        assert_eq!(
            affected.build_disposition,
            crate::codex::domain::BuildDisposition::Replaced,
...
fn t_s04_051_durable_carry_pages_resume_after_reopen_and_finalize_only_at_end() {
    ...
    for target in [
        ...
        loop {
            ...
            let reopened =
                Ledger::open(LedgerOptions::new(fixture.root.join("mu.sqlite3"))).unwrap();
            with_codex(&reopened, |storage| {
                storage.resume_carry(1, 1_000 + page_count)
            })
            .unwrap();
            drop(reopened);
            page_count += 1;
    ...
    assert_eq!(
        with_codex(&reopened, |storage| storage.resume_carry(1, 1_200)).unwrap(),
        CarryStepOutcome::FinalizedMissing
    );
