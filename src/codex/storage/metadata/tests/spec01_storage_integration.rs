...
use super::with_codex;
use crate::codex::domain::{
    AgentPathProvenance, AgentRoleProvenance, CheckpointProcessingStatus, ContinuationState,
    CwdProvenance, FactQualityStatus, MetadataCheckpointAdvance, MetadataCommitBatch,
    MetadataSourceCommit, MetadataThreadCommit, OwnershipConfidence, ParentHintProvenance,
    RolloutMetadataFact, SafeFactMismatchReason, SafeFactState, SourceArea, SourceObservation,
    SourceObservationBatch, SourceRegionStatus,
};
use crate::domain::{AgentRole, Patch, ResolvedThreadPatch, SessionIdentity};
...
impl TempRoot {
    ...
    fn ledger(&self) -> Ledger {
        Ledger::open(LedgerOptions::new(self.db_path())).expect("open temporary ledger")
    }
}
...
fn bootstrap_matching_fact(
    ...
    connection
        .execute(
            "INSERT OR REPLACE INTO codex_rollout_metadata_facts (
...
fn t_s01_001_v1_schema_initial_state_pragmas_and_reopen_matrix() {
    ...
    v1.execute_batch(include_str!(concat!(
    ...
    let state = with_codex(&reopened, |storage| {
...
fn t_s01_002_source_identity_database_constraints_matrix() {
    ...
    assert_eq!(
        connection
            .query_row("SELECT count(*) FROM codex_source_files", [], |row| row
                .get::<_, i64>(0))
            .unwrap(),
        1
    );
...
fn t_s01_005_safe_fact_reuse_mismatch_matrix() {
    ...
    connection
        .execute(
            "UPDATE codex_rollout_metadata_facts SET file_generation = 2 WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    drop(connection);
    ...
    connection
        .execute(
            "UPDATE codex_rollout_metadata_facts SET metadata_parser_version = 6 WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    drop(connection);
    ...
    connection
        .execute(
            "UPDATE codex_rollout_metadata_facts SET resolved_through_offset = 63 WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    drop(connection);
    ...
    connection
        .execute(
            "UPDATE codex_source_files SET thread_id = NULL WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    drop(connection);
    ...
    connection
        .execute(
            "UPDATE codex_source_files SET thread_id = 'thread-2' WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    drop(connection);
...
fn t_s01_008_batch_metadata_state_is_one_sqlite_snapshot() {
    ...
    while reads < 300 || !done.load(Ordering::Acquire) {
        let state = with_codex(&ledger, |storage| storage.load_metadata_scan_state(&ids)).unwrap();
...
fn t_s01_009_metadata_transaction_rollback_survives_reopen() {
    ...
    connection
        .execute_batch(
            "CREATE TRIGGER spec01_abort_metadata_checkpoint
             BEFORE UPDATE ON codex_source_checkpoints
    ...
    let identity =
        SessionIdentity::new("thread-rollback", SourceId::CODEX, "thread-rollback").unwrap();
    ...
    let result = with_codex(&ledger, |storage| {
        storage.commit_metadata(
            MetadataCommitBatch::new(vec![
    ...
    connection
        .execute(
            "UPDATE codex_source_files SET thread_id = 'thread-crash' WHERE source_file_id = ?1",
            [crash_source],
        )
        .unwrap();
    connection
    ...
    connection
        .execute(
            "UPDATE codex_source_checkpoints SET parser_version = 1, committed_offset = 80,
                    guard_hash = X'01', processing_status = 'ready'
             WHERE source_file_id = ?1 AND consumer_kind = 'metadata'",
            [crash_source],
        )
    ...
    let reopened = Ledger::open(LedgerOptions::new(&crash_db)).unwrap();
    let connection = Connection::open(reopened.database_path()).unwrap();
    let thread_id: Option<String> = connection
        .query_row(
            "SELECT thread_id FROM codex_source_files WHERE source_file_id = ?1",
...
fn t_s01_013_deleting_source_cascades_all_consumer_checkpoints() {
    ...
    assert_eq!(
        connection
            .query_row(
                "SELECT count(*) FROM codex_source_checkpoints WHERE source_file_id = ?1",
                [source_file_id],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),
    ...
    connection
        .execute(
            "DELETE FROM codex_source_files WHERE source_file_id = ?1",
            [source_file_id],
        )
        .unwrap();
    assert_eq!(
...
fn t_s01_025_schema_and_test_fixtures_enforce_privacy_boundary() {
    ...
    for module in ["storage", "scanner", "codex", "usage"] {
        ...
        for path in module_files {
            ...
            for macro_name in forbidden_logging_sinks {
                assert!(
