...
use crate::{
    ...
    codex::storage::source_state::UsageCarryObservationProof,
    codex::storage::{CodexStorage, CodexStorageError},
    codex::{
...
/// to `CodexStorage::commit_metadata`.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl MetadataPipeline {
    ...
    pub fn record_and_load(
        ...
        self.record_and_load_with_usage_carry_proofs(storage, snapshot, &[])
    }
    ...
    pub(crate) fn record_and_load_with_usage_carry_proofs(
        ...
        let outcome = storage
            .record_source_observations_with_usage_carry_proofs(batch, usage_carry_proofs)?;
        let ids = outcome
            .results
            .iter()
            .map(|result| result.source_file_id)
            .collect::<Vec<_>>();
        let state = storage.load_metadata_scan_state(&ids)?;
        Ok((outcome, state))
    }
    ...
    fn plan_one(
        &self,
        file: &DiscoveredFile,
        outcome: &crate::codex::domain::SourceObservationResult,
        ...
        let resume_state = stable_fact.and_then(|fact| {
            ...
            match fact.continuation_state {
                ...
                crate::codex::domain::ContinuationState::OwningLive => {
                    Some(ResumeState::OwningLive {
                        owning_thread_id: thread_id.clone(),
                    })
                }
                crate::codex::domain::ContinuationState::Unstable => None,
            }
        });
    ...
    pub fn resolve(
        ...
        for thread_id in thread_ids {
            let source_file_ids = source_view
                .iter()
                .filter(|source| source.file_status == crate::codex::domain::FileStatus::Present)
                .filter(|source| {
                    source
                        .thread_id
                        .as_deref()
    ...
    pub fn commit(
    ...
    ) -> Result<Option<crate::codex::domain::CommitOutcome>, PipelineError> {
        ...
        let outcome = crate::codex::domain::CommitOutcome::new(
            committed_group_count,
            data_revision,
            data_changed,
        )
...
mod tests {
    ...
    struct TempCodexStorage {
        directory: PathBuf,
        ledger: Arc<Ledger>,
        storage: SourceStorage,
    }

    impl TempCodexStorage {
        fn new() -> Self {
            ...
            let ledger =
                Arc::new(Ledger::open(LedgerOptions::new(directory.join("mu.db"))).unwrap());
            ledger
                .connection()
                .unwrap()
                .execute(
        ...
        fn codex(&self) -> CodexStorage<'_> {
            CodexStorage::new(&self.storage).unwrap()
        }
    }

    impl Drop for TempCodexStorage {
        fn drop(&mut self) {
    ...
    fn discovered(path: &str, device_id: i64, inode: i64, size: i64, mtime: i64) -> DiscoveredFile {
        DiscoveredFile {
            ...
            source_area: if path.contains("archived") {
                crate::codex::domain::SourceArea::ArchivedSessions
            } else {
    ...
    fn identity_lifecycle_matrix_preserves_moves_and_rebuilds_content_generations() {
        let fixture = TempCodexStorage::new();
        let pipeline = MetadataPipeline::new(1, 10).unwrap();
        ...
        let (first, _) = pipeline
            .record_and_load(&fixture.codex(), &first_snapshot)
            .unwrap();
        let source_id = first.results[0].source_file_id;
        let connection = Connection::open(fixture.ledger.database_path()).unwrap();
        connection
            .execute(
                "UPDATE codex_source_files SET thread_id = 'thread-a' WHERE source_file_id = ?1",
                [source_id],
            )
            .unwrap();
        connection
            .execute(
                "UPDATE codex_source_checkpoints SET parser_version = 1, committed_offset = 5,
        ...
        let (renamed_outcome, renamed_state) = pipeline
            .record_and_load(&fixture.codex(), &renamed)
            .unwrap();
        ...
        let usage_before_rewrite: (String, i64) = connection
            .query_row(
                "SELECT processing_status, committed_offset FROM codex_source_checkpoints
        ...
        let (copy_outcome, _) = pipeline.record_and_load(&fixture.codex(), &copied).unwrap();
        ...
        let (replacement, replacement_state) = pipeline
            .record_and_load(&fixture.codex(), &replaced)
            .unwrap();
        ...
        let (truncation, _) = pipeline
            .record_and_load(&fixture.codex(), &truncated)
            .unwrap();
        ...
        let (rewrite, _) = pipeline
            .record_and_load(&fixture.codex(), &rewritten)
            .unwrap();
        assert_eq!(rewrite.results[0].file_generation, 4);
        assert!(rewrite.results[0].replaced);

        pipeline
            .record_and_load(&fixture.codex(), &snapshot(Vec::new()))
            .unwrap();
        let missing = fixture
            .codex()
        ...
        let (restored_outcome, restored_state) = pipeline
            .record_and_load(&fixture.codex(), &restored)
            .unwrap();
        ...
        pipeline
            .record_and_load(&fixture.codex(), &unavailable)
            .unwrap();
